...
class Worker(QThread):
    ...
    #result = pyqtSignal(datetime64, tuple)
    #start serial com
...
class MainWindow(QMainWindow):
    ...
    def applySerialPort(self):
        #need to add dev infront...
        #tty works better than cu
    ...
    def updateData(self, current_time, temperatures):
        ...
        #formattime = dt.fromtimestamp(current_time).strftime('%H:%M:%S')
        formattime = dt.strptime(current_time, '%H:%M:%S')
        #formattime = (np.asarray(current_time).astype(float)).astype('datetime64[s]')
        ...
        #self.plot.getAxis('bottom').setTicks([[(t, dt.fromtimestamp(t).strftime('%H:%M:%S')) for t in self.time]])
